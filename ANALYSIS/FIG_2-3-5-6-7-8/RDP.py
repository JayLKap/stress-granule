...
class RDP:
    def __init__(self, density_file, pca_file, cluster_file, T, init, label):
        ...
        self.distances = density_profile.iloc[1:, 0].tolist()
        densities = np.array(density_profile.iloc[1:, 1].tolist())
        #max_dens = np.max(densities)
        self.densities = list(densities)
        self.sig = density_profile.iloc[1:, 2].tolist()
        self.errors = list(np.array(density_profile.iloc[1:, 2].tolist()))
